...
function initApp(config, redirectHttp) {
    const { respondWith, sslPort, port } = config
    const app = express()
    ...
    if (respondWith) {
        ...
        // console.log(!ssl && redirectHttp, redirectHttp)
        app.get('/*', (req, res) => {
            if (redirectHttp) {
              let fullUrl = req.headers.host + req.url
              fullUrl = fullUrl.substring(0, fullUrl.length - port.toString().length - 1)
              res.redirect('https://' + fullUrl + sslPort)
            } else {
...
function createServer(config, proxy, redirectHttp = false) {

    const app = initApp(config, redirectHttp)
